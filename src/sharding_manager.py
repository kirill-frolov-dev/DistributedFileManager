...
class ShardingManager:
    def __init__(
        self, disk_repo: DiskRepositoryProtocol, obj_repo: ObjectRepositoryProtocol
    ):
    ...
    def allocate_disk(self, required_bytes: int) -> int:
        ...
        raise NotEnoughSpaceError(
            "Нет подходящего диска с достаточным свободным местом"
        )
    ...
    def reserve_space(self, disk_id: int, bytes_to_reserve: int) -> None:
        ...
        if disk["status"] != "available":
            raise DiskUnavailableError(
                f"Диск {disk_id} недоступен для записи (статус: {disk['status']})"
            )
        self.disk_repo.update_disk_space(
            disk_id, used_delta=0, reserved_delta=bytes_to_reserve
        )
    ...
    def complete_object(self, obj_id: str) -> None:
        ...
        self.disk_repo.update_disk_space(
            obj["disk_id"], used_delta=0, reserved_delta=-obj["reserved_size"]
        )
        # Обновляем объект
    ...
    def expand_reserved(self, obj_id: str, extra_bytes: int) -> bool:
        ...
        self.disk_repo.update_disk_space(
            obj["disk_id"], used_delta=0, reserved_delta=extra_bytes
        )
    ...
    def set_object_size(self, obj_id: str, actual_size: int) -> None:
        ...
        self.disk_repo.update_disk_space(
            disk_id=disk_id, used_delta=delta, reserved_delta=0
        )
