...
class SqliteDiskRepository:
    ...
    def update_disk_space(
        self, disk_id: int, used_delta: int, reserved_delta: int
    ) -> (
        None
    ):  # used_delta - изменение used_space, reserved_delta - изменение reserved_space
        cur = self.conn.cursor()
        cur.execute(
            """UPDATE disks 
                    SET used_space = used_space + ?, reserved_space = reserved_space + ? 
                    WHERE id = ?""",
            [used_delta, reserved_delta, disk_id],
        )
        self.conn.commit()
