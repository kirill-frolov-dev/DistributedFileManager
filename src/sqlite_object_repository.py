...
class SqliteObjectRepository:
    ...
    def _init_tables(self):
        ...
        cur.execute("""CREATE TRIGGER update_objects_updated_at 
    ...
    def add_object(self, obj_id: str, disk_id: int, reserved_size: int) -> None:
        cur = self.conn.cursor()
        cur.execute(
            """INSERT INTO objects (id, disk_id, reserved_size) VALUES (?, ?, ?)""",
            (obj_id, disk_id, reserved_size),
        )
        self.conn.commit()
    ...
    def get_incomplete_objects(self, older_than_seconds: int) -> list[dict]:
        ...
        rows = cur.execute(
            """SELECT * 
                    FROM objects 
                    WHERE is_completed = 0 AND updated_at < datetime('now', ?)""",
